...
bed_coords1 = bind_rows(SpectralTAD(rao_chr22_prim, chr = "chr22", levels = 3))
...
We provide tools for visualization using the `DiffPlot` function. As an input, `DiffPlot` takes the output from the `TADCompare` function and the original contact matrices, providing a visualization of differential TADs, TAD scores and the matrices themselves. As an output, it returns a ggplot object that can be further customized, if necessary. We demonstrate visualization of the differences between GM12878 and IMR90 cell lines, on the subset of chromosome 2, 40kb resolution data processed in Schmitt et al. [@Schmitt:2016aa]:
...
From these results, we can see that boundary scores from both matrices ("Boundary score 1" and "Boundary score 2") frequently detected as significant boundaries in both matrices (boundary scores above 1.5 threshold), but are non-differential (black dots). The differential boundaries correspond to the "Differential boundary score" track, where absolute boundary score differences above 2.0 threshold are detected. The different types of differential boundaries are defined according to a schema described in the TADCompare manuscript. 
...
bed_coords2 = bind_rows(SpectralTAD(mtx2, chr = "chr2", levels = 3))
...
As we can see, the pre-specification of TADs allows us to outline TAD boundaries enhancing visualization. Additionally, the differential boundaries presented now correspond to those called by the TAD caller as opposed to those detected by TADCompare using boundary scores. Given that the pre-specified TAD boundaries are not necesserily correspond to the boundary scores, the classification schema of the differences shows whether a boundary is differential or not. Additionally, a new category is introduced, called "Non-Overlap". Non-Overlap corresponds to boundaries that are determined to be non-differential by TADCompare but do not overlap when called by the TAD caller.
