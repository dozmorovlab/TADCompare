...
MultiCompare = function(cont_mats, resolution,
                      z_thresh = 2, window_size = 25, gap_thresh = .5,
                      plot = FALSE,
                      start = 0, end = 2e6, Pre_Def = TRUE,
                      consensus_thresh = floor(length(cont_mats)/2),
                      consensus_bounds = TRUE, groupings = NULL) {
  ...
  #Call TADs
  if (Pre_Def == FALSE) {
    TADs = lapply(cont_mats, function(x) {
      SpectralTAD(x, chr = "chr", levels = 2, z_clust = FALSE,
                  qual_filter = FALSE, resolution = resolution)
    })

    #Get shared ends
    TAD_Ends = unique(do.call(bind_rows, TADs)$end)

    score_frame = score_frame %>% filter(Coordinate %in% TAD_Ends)
  }
  ...
  sim_mat = cor(TAD_Frame[,-c(1, ncol(TAD_Frame))])
...
Single_Dist = function(cont_mat1, resolution, window_size = 25, gap_thresh = .5) {
