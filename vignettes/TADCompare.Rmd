...
TADCompare is a function that allows users to automatically identify differential TAD boundaries between two datasets. For each differential boundary, we provide unique classification (complex, merge, split, shifted and strength change) defining how the TAD changes between the datasets.
...
TADCompare returns a list with 2 data frames and 1 plot. The two data frames are TAD_Frame which contains all regions with at least one TAD. Its columns correspond to the coordinates of the TAD boundary, the differential boundary score between both data sets, a general designation of differential or non-differential, the contact matrix which contains the TAD boundary and the type of TAD boundary. The results are shown below.

```{r}
head(results$TAD_Frame)
```

The second data frame contains coordinates, differential boundary scores and indvidual boundary scores for each region of the contact matrix regardless of whether it contains a TAD boundary. 

```{r}
head(results$Gap_Scores)
```
...
To demonstrate one of the ways to utilize TADCompare, we provide a walkthrough of performing gene ontology enrichment analysis using TADCompare and `rGREAT` (Version 2.0). We first show how to perform gene ontology enrichment on shifted boundaries enriched in matrix 1.
...
Now we demonstrate how to perform the same analysis but for all boundary types simultaneously. For this example we still focus on boundaries enriched in matrix 2. 
