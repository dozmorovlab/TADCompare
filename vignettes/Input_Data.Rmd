...
`TADCompare` is an R package for differential analysis of TAD boundaries. It is designed to work on a wide range of formats and resolutions of Hi-C data. `TADCompare` package contains three functions: `TADCompare`, `TimeCompare` and `ConsensusTADs`. `TADCompare` allows for identification of differential TAD boundaries between two contact matrices. [??? Link to the corresponding vignette] `TimeCompare` takes a set of contact matrices, one matrix per time point, identifies TAD boundaries and classifies how they change over time. [??? Link to the corresponding vignette] `ConsensusTADs` takes a list of TADs and identifies a consensus of TAD boundaries across all matrices using our novel consensus boundary score. [??? Link to the corresponding vignette] The required input includes matrices in sparse 3-column format, $n \times n$, or $n \times (n+3)$ formats. [??? TBD the following] Instructions on changing the default parameters are provided. 
...
Users can also find TADs from data output by `cooler` (http://cooler.readthedocs.io/en/latest/index.html) and HiC-Pro (https://github.com/nservant/HiC-Pro) with minor pre-processing using the `HiCcompare` package. [??? Link to the relevant resources]
...
[??? Does not work, IOError: Unable to open file (unable to open file: name = 'ftp://cooler.csail.mit.edu/coolers/hg19/Zuin2014-HEK293CtcfControl-HindIII-allreps-filtered.50kb.cool', errno = 2, error message = 'No such file or directory', flags = 0, o_flags = 0) ??? Add wget download command ??? Select example data that you are using in the vignette]
2. Convert the first matrix to a text file using `cooler dump --join ftp://cooler.csail.mit.edu/coolers/hg19/Zuin2014-HEK293CtcfControl-HindIII-allreps-filtered.50kb.cool > Zuin.HEK293.50kb.Control.txt`
3. Convert the first matrix to a text file using `cooler dump --join ftp://cooler.csail.mit.edu/coolers/hg19/Zuin2014-HEK293CtcfDepleted-HindIII-allreps-filtered.50kb.cool > Zuin.HEK293.50kb.Depleted.txt`
...
[??? Select better data, describe where it came from, include in the package]
```{r, eval = FALSE}
#Read in both files
...
The table above shows a summary of runtimes for different types of contact matrices measured in seconds. [??? Show only "mean" time] As we see, `TADCompare` is extremely fast no matter the parameters. However, sparse matrix inputs will slow down the algorithm. This can become more apparent as the size of the contact matrices increase.
