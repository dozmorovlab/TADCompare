...
"Region" corresponds to the region of the genome. "TAD_Score1" and "TAD_Score2" correspond to boundary scores for each contact matrix, and "Gap_Score" is the differential boundary score. Note: The first boundary will always be classified as "NA" due to the fact that specific classification is impossible without a reference boundary to the left and right.
...
# Error in ifelse((Differential == "Differential") & (Bound_Dist <= 5) &  : 
#  object 'Bound_Dist' not found

#Returning the boundaries
tail(TD_Compare$TAD_Frame)
#Testing to show that pre-specified boundaries are returned
table(TD_Compare$TAD_Frame$Boundary %in% bind_rows(Combined_Bed)$end) # ??? This doesn't work, return FALSE
# # ??? The below doesn't work either
# # ??? TADCompare seem to return different boundaries than pre-specified
# table(TD_Compare$TAD_Frame$Boundary %in% bind_rows(Combined_Bed)[, "end"] %>% unlist() %>% unname)
# table(TD_Compare$TAD_Frame$Boundary %in% bind_rows(Combined_Bed)[, "start"] %>% unlist() %>% unname)
```

Here, we see that the only boundaries that are returned are those we pre-specified. [??? NO!]
...
rao_chr22_prim =as.matrix(read.table("file:///C:/Users/cresswellkg/Documents/differentialTADs/Data/npc.rep1.nor.chr22.mat"))

rao_chr22_rep = as.matrix(read.table("file:///C:/Users/cresswellkg/Documents/differentialTADs/Data/mes.rep2.nor.chr22.mat"))

colnames(rao_chr22_prim) = colnames(rao_chr22_rep)= seq(0, (nrow(rao_chr22_prim)-1)*40000, 40000 )


#Calling differential TADs
results = TADCompare(rao_chr22_prim, rao_chr22_rep, resolution = 40000)
...
DiffPlot(tad_diff    = results, 
         ...
         resolution  = 40000,
         start_coord = 25000000
,
         end_coord   = 27560000
)
```

From these results, we can see that the vast majority of regions are below our threshold of 1.5 for TAD boundary detection (First two horizontal red lines).
...
DiffPlot(tad_diff    = TD_Compare, 
         ...
         cont_mat2   = rao_chr22_rep,
         resolution  = 40000,
         start_coord = 24360000,
         end_coord   = 26600000,
         pre_tad     = Combined_Bed)
```

As we can see, the pre-specification of TADs allows us to outline TAD boundaries enhancing visualization. Additionally, the differential boundaries presented now correspond to those called by the TAD caller as opposed to those detected by TADCompare. Additionally, a new category is introduced, "Non-Overlap". Non-Overlap corresponds to regions that are determined to be non-differential by TADCompare but do not overlap when called by the TAD caller.
